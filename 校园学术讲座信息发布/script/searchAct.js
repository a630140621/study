...
apiready = function() {
    ...
    if (cityCon) {
        var getAllCityUrl = '/city';
        ajaxRequest(getAllCityUrl, 'GET', '', function (ret, err) {
            if (ret) {
    ...
    if (typeCon) {
        var getAllTypeUrl = '/type?filter=';
        var urlParam={
            where:{category:4}
        }
        ajaxRequest(getAllTypeUrl+JSON.stringify(urlParam), 'GET', '', function (ret, err) {
            if (ret) {
